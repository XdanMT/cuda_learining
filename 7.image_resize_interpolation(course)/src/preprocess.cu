    ...
    float scaled_w, float scaled_h) 
{
    ...
    if (src_y1 < 0 || src_x1 < 0 || src_y1 > srcH || src_x1 > srcW) {
        ...
        float a1_2 = (1.0 - tw) * th;    // 感觉a1_2和a2_1写反了，a1_1才是tw * (1.0 - th)？
    ...
    int srcW, int srcH)
{
    ...
    dim3 dimGrid(tarW / 16 + 1, tarH / 16 + 1, 1);  // grid_size可能会多1，但是不影响kernel的计算，因为在kernel内部会将坐标无效的thread过滤掉
...
// 模板显式实例化，个模板实例会在编译期生成，之后的调用不会再实例化新代码，
