...
__global__ void matrix_mul_kernel(double *M_device, double *N_device, double *P_device, int width) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x < width && y < width) {
        double element_sum = 0.0;
        for (int k = 0; k < width; k++) {
            double a = M_device[y * width + k];
            double b = N_device[k * width + x];
            element_sum += a * b;
        }
...
void matrix_mul_gpu(double* M_host, double* N_host, double* P_host, int width, int blk_size) {
    double *M_device, *N_device, *P_device;   // device上的输入矩阵和输出矩阵

    // 申请device上的内存
    cudaMalloc((void**)&M_device, width * width * sizeof(double));
    cudaMalloc((void**)&N_device, width * width * sizeof(double));
    cudaMalloc((void**)&P_device, width * width * sizeof(double));

    // 将host上的输入矩阵拷贝到device上
    cudaMemcpy(M_device, M_host, width * width * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(N_device, N_host, width * width * sizeof(double), cudaMemcpyHostToDevice);
    ...
    CUDACHECK(cudaMemcpy(P_host, P_device, width * width * sizeof(double), cudaMemcpyDeviceToHost));
    LAST_KERNEL_CHECK();
