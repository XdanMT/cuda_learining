...
__global__ void matrix_mul_kernel_shared_static_with_bank_conflict(float *M_device, float *N_device, float *P_device, int width,int blk_size) {
...
__global__ void matrix_mul_kernel_shared_dynamic_with_bank_conflict(float *M_device, float *N_device, float *P_device, int width,int blk_size) {
    /* 
    ...
    */
    ...
    if (x < width && y < width) {
        for (int tile_idx = 0; tile_idx < width / blk_size; tile_idx++) {
            ...
            shared_mem[ty * blk_size + tx] = M_device[y * width + (tile_idx * blk_size + tx)];
            shared_mem[(ty * blk_size + tx) + shared_mem_stride] = N_device[((tile_idx * blk_size + ty)) * width + x];
            __syncthreads();

            for (int k = 0; k < blk_size; k++){
                element_sum += shared_mem[ty * blk_size + k] * shared_mem[(k * blk_size + tx) + shared_mem_stride];
            }
            __syncthreads();
        }
        P_device[y * width + x] = element_sum;
...
void matrix_mul_gpu_shared_with_bank_conflict(float* M_host, float* N_host, float* P_host, int width, int blk_size, bool use_static_shared_memory) {
    ...
    if (use_static_shared_memory){
        matrix_mul_kernel_shared_static_with_bank_conflict<<<grid_size, block_size>>>(M_device, N_device, P_device, width, blk_size);
    }
    else{
        matrix_mul_kernel_shared_dynamic_with_bank_conflict<<<grid_size, block_size, blk_size * blk_size * sizeof(float)*2, nullptr>>>(M_device, N_device, P_device, width, blk_size);
    }
